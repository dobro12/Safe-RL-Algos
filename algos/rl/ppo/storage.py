...
class RolloutBuffer:
    ...
    def addTransition(self, states, actions, rewards, dones, fails, next_states):
        for env_idx in range(self.n_envs):
            self.storage[env_idx].append([
                states[env_idx], actions[env_idx], rewards[env_idx], 
    ...
    def getBatches(self, obs_rms, reward_rms, reward_critic):
        ...
        for env_idx in range(self.n_envs):
            ...
            fails = np.array([traj[4] for traj in env_trajs])
        ...
        return states_tensor, actions_tensor, reward_targets_tensor, reward_gaes_tensor
