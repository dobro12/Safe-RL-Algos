...
class RolloutBuffer:
    ...
    def getBatches(self, obs_rms, reward_rms, reward_critic, cost_critic):
        ...
        for env_idx in range(self.n_envs):
            ...
            constraints_list.append(np.mean(costs, axis=0)/(1.0 - self.discount_factor))
        ...
        constraints_tensor = torch.tensor(np.mean(constraints_list, axis=0), device=self.device, dtype=torch.float32)
