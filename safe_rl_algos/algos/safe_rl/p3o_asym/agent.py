...
class Agent(AgentBase):
    def __init__(self, args) -> None:
        ...
        self.state_dim = args.__dict__.get('state_dim', 0)
        ...
        # for normalization
        self.state_rms = RunningMeanStd('agent_state', self.state_dim)
        ...
        self.reward_critic = RewardCritic(
            self.device, self.obs_dim + self.state_dim, self.model_cfg['reward_critic']).to(self.device)
        self.cost_critic = CostCritic(
            self.device, self.obs_dim + self.state_dim, self.cost_dim, self.model_cfg['cost_critic']).to(self.device)
        ...
        self.replay_buffer = RolloutBuffer(
            self.device, self.obs_dim, self.state_dim, self.action_dim, self.discount_factor, 
    ...
    @torch.no_grad()
    def getAction(self, obs:np.ndarray, state:np.ndarray, deterministic:bool) -> np.ndarray:
        obs_tensor = torch.tensor(self.obs_rms.normalize(obs), dtype=torch.float32, device=self.device)
        epsilon_tensor = torch.randn(obs_tensor.shape[:-1] + (self.action_dim,), device=self.device)

        self.actor.updateActionDist(obs_tensor, epsilon_tensor)
    ...
    def step(self, rewards, costs, dones, fails, next_obs, next_states) -> None:
        self.replay_buffer.addTransition(
            self.obs, self.state, self.action, rewards, costs, 
            dones, fails, next_obs, next_states)

        # update statistics
        if self.norm_obs:
            self.obs_rms.update(self.obs)
            self.state_rms.update(self.state)
        if self.norm_reward:
    ...
    def train(self):
        # get batches
        with torch.no_grad():
            obs_tensor, states_tensor, actions_tensor, reward_targets_tensor, cost_targets_tensor, \
            reward_gaes_tensor, cost_gaes_tensor, con_vals = \
                self.replay_buffer.getBatches(
                    self.obs_rms, self.state_rms, self.reward_rms, self.reward_critic, self.cost_critic)

        # ================== Critic Update ================== #
        critic_states_tensor = torch.cat([obs_tensor, states_tensor], dim=-1)
        for _ in range(self.n_critic_iters):
            reward_critic_loss = self.reward_critic.getLoss(critic_states_tensor, reward_targets_tensor)
            self.reward_critic_optimizer.zero_grad()
            reward_critic_loss.backward()
            torch.nn.utils.clip_grad_norm_(self.reward_critic.parameters(), self.max_grad_norm)
            self.reward_critic_optimizer.step()

            cost_critic_loss = self.cost_critic.getLoss(critic_states_tensor, cost_targets_tensor)
        ...
        with torch.no_grad():
            ...
            self.actor.updateActionDist(obs_tensor, epsilons_tensor)
